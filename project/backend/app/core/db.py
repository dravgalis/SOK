...
def _connect() -> sqlite3.Connection:
    ...
    connection = sqlite3.connect(settings.users_db_path)
    ...
    with _connect() as connection:
        ...
        if existing is None:
            connection.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                ''',
                (hh_id, name, email, now_iso, now_iso),
            )
        else:
            connection.execute(
                '''
...
def get_all_users() -> list[dict[str, str | None]]:
    with _connect() as connection:
        rows = connection.execute(
            '''
            SELECT hh_id, name, email, created_at, last_login
            FROM users
